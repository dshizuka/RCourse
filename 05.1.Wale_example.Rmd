---
title: 'Module 8: Effect of anthropogenic noise on crab physiology' 
author: "Dai Shizuka"
...
![Shore crab, *Carcinus maenas*](images/crab.jpg){width="20%"}
...
```{r, out.width="50%"}
setwd("~/Dropbox/dai_teaching/Intro_To_R_2017/Exercise-Wale2013")
single=read.csv("single_exposure.csv")
head(single)

mult=read.csv("multiple_exposure.csv")
head(mult)
```

***

##8.3 Testing the relationships between mass, noise, and physiology
...
###8.3.1 Plotting Figure 1

We can plot the relationships between these three things this way:

```{r, out.width='50%', echo=F, fig.cap="*Figure 1: The relationship between body mass and oxygen consumption in shore crabs. Individuals in the 'Ambient' treatment shown in open circles, individuals in 'Ship' treatment shown in filled diamonds.*"}
...
> **Exercise: Write the code to create this figure!**
...
###8.3.3: Table of basic statistical tests
...
##8.4 Examining the effect of repeated exposure to noise
...
###8.4.2 Conducting the repeated-measures ANOVA
...
###8.4.3: A little bit of a correction on the paper:
...
###8.4.4: Accounting for repeated measures using a ***linear mixed-effects model***
...
###8.5.2 A fancier way to summarize data using `dplyr`
...
###8.5.3 Plotting the figure with `ggplot2`
