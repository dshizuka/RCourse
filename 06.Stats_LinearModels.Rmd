...
```{r}
lm.fit=lm(mpg~hp, mtcars)
glm.fit=glm(mpg~hp, mtcars, family="gaussian")
```
...
## 5. Multiple predictor variables in a Linear Model
...
## 4. GLMs with binary, count and proportion data as response variable
...
diamonds
```
Let's make a new variable called "expensive", which is TRUE if the diamond is priced at or above $10,000, and FALSE if not.
```{r}
...
### 4.2. GLM with proportion data

Here we will use the "snail" dataset from the "MASS" package
```{r}
snails
```

```{r}
prop.fit=glm(Deaths~Temp, data=snails, family="poisson")
summary(prop.fit)
```

```{r}
ggplot(snails, aes(x=Temp, Deaths)) +
  geom_count() +
