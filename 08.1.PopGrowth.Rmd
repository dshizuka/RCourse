---
title: 'Module 10: Modeling Population Growth' 
author: "Dai Shizuka"
...
Let's also install and load packages that we'll use.
```{r, eval=F, message=F}
...
##9.1 Modeling the basic exponential/geometric population growth model
...
```{r, echo=T, eval=F, fig.width=4, fig.height=3, out.height='2in'}
plot(t, N, type="o", pch=19, las=1)
```
```{r, echo=F, fig.width=4, fig.height=3, out.height='2in'}
par(mar=c(4,4,1,4))
plot(t, N, type="o", pch=19, las=1)
```
...
**Note:** *Compare this with this alternative method to plot y on the log-scale (base 10 log). Clue: pay attention to the y-axis tick marks.*
```{r, echo=T, eval=F, fig.width=4, fig.height=3, out.height='3in'}
#output not shown
plot(t, log10(N), type="o", pch=19, las=1)
```
...
```{r, fig.width=4, fig.height=6, out.height='3in'}
...
##9.4 Population Viability Analysis: An over-simplified example
...
###9.4.1 The Song Sparrow Dataset
...
###9.4.2 Calculating lambda from counts
...
###9.4.3 Projecting the population over 20 years
...
Using the same principles as the stochastic simulations in Section 9.3, we can also conduct multiple simulations and plot the results
```{r, fig.height=3.5, fig.width=4.5}
set.seed(2)
sims=5
outmat=sapply(1:sims, function(x){
...
###9.4.4 The PVA: What is the extinction risk?
