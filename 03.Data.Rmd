...
>
>* Click on the following links to download three files: ["SampleData.csv"](data/SampleData.csv), ["SampleData_w_errors.csv"](data/SampleData_w_errors.csv), ["sex_chromo.csv"](data/sex_chromo.csv)
>
><a href="data/SampleData.csv" download> or this link </a>
...
###3.2 Importing .csv (or .txt) files
...
####Five ways to import data from .csv file:

####1. RStudio Project (most recommended)
...
```
####2. Entering the full path name
Using the above example location for the file, you can use either of the below versions of the pathname to import the data. ***Remember: You will have to change the path inside the lines of code here to be where the file resides in your computer.
...
####3. Use a url (recommended for data downloadable from the web)

  
####4. Call 'choose file' prompt. 
