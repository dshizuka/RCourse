...
We can extract all of the values that are reported by in the `summary()` function (i.e., rather than copying them down by hand). But this takes a little sleuthing. To get started, let's look at what is actually included in the output of the `summary()` function by using the `str()` function: 
...
Then, you see that actually, this contains three values: (1) The F-value, and (2,3) are the degrees of freedom of the model. We want to save all of these, so we do:
...
First thing we need to do to construct this figure is to calculate the mean wing lengths for each year for each species and sex. We can do this with `group_by()` and `summarize()` functions that we used in a previous module:
...
### 4.2 Creating a custom function to calculate standard error of the mean
...
So, what we need to do is figure out an easy way to do this calculation for each year-species-sex. The best way to do this is to ***build a custom function*** to calculate the standard error, and then use this function within the `summarize()` function (like we did to calculate the means)!
...
Note: I use `na.rm=T` inside the `mean()` function to skip NAs when calculating the mean, and I take the length of values that are NOT NAs for the denominator:
...
### 4.3 Plotting the standard error bars on the line plots
...
```{r}
wl.boloria=boloria %>% group_by(year, sex) %>% summarise(mean.WL=mean(WL), se.WL=se(WL)) 
```
...
*You'll notice that there is the data for 2003 is missing for females.* This is because there is a missing data point, and therefore the `mean()` function is returning NA. We can get around this by telling the `mean()` function to skip the NA using the argument `na.rm=T`. So redo the plot:
```{r}
...
Now, we want to combine the winglength data for each species with the snowmelt/temperature data. We can do that using a `_join()` function in dplyr. Here, we'll use `left_join()`:
```{r}
