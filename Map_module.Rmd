...
```{r}
co_data=read.csv("data/cc-est2024-alldata-31.csv")
head(co_data)
```

```{r}
co_data_allage2024=co_data %>% filter(AGEGRP==0 & YEAR==4)
```
...
```{r}
ne_full2024=ne_shp %>% mutate(CountyFIPS=as.numeric(CountyFIPS)) %>% left_join(co_data_allage2024, by=join_by("CountyFIPS"=="COUNTY"))
```
...
```{r}
plot(ne_full2024["TOT_POP"])
```

```{r}
ggplot() +
  geom_sf(data=ne_full2024, aes(fill=TOT_POP)) +
  theme_bw()
```
