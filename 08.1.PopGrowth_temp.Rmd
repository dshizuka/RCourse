---
title: 'Module 8 Example: Modeling Population Growth' 
author: "Dai Shizuka"
...
##5. Population viability with carrying capacity

Real populations are under density-dependent regulation. So let's go a step further and incorporate carrying capacity in our model. 

To do that, we first want to build in the logistic population growth model, where carrying capacity is *K*. In this case, the rate of change of a population at any time *t* is described the ***logistic population growth equation***:
$$\frac{dN}{dt}=rN\left(1-\frac{N}{K}\right)$$
The population changes more (increases faster) when *N* is small, and stops growing when *N* approaches *K*.

To express population size at any time *t*, you can use this equation:
$$N_t=N_{t-1}+rN_{t-1}\left(1-\frac{N}{K}\right)$$
That is, the population size at time *t* is the population size in the previous time step (*t-1*) plus the change in the population.

So let's say we have a population with $K=50$, and initial population size $N_0=10$.

We can project the growth of this population like this:
...
Ok, so now let's go back to the real sparrow data again:

```{r}
plot(Count~Year, data=sparrows, type="o", las=1)
```

I'm not going to formally fit a model and calculate carrying capacity here... but let's just say that the population seems to be fluctuating around 50 indivudals or so. So if we set $K=50$
