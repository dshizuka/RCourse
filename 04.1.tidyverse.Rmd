...
## 2. Some things to know about getting started with 'tidyverse'
...
### 2.3 Installing tidyverse packages (and packages in general)
...
Note that this simply downloads the packages onto your computer. When you are ready to use them, you will have to load the package onto the environment by running the function `library()` or `require()`. We will do this in the next section.

***


### 2. Loading the package and getting oriented

Let's start by loading the package and finding out a bit about what is in the package. We can load the entire `tidyverse` package (or, if you prefer, you can just load the `tidyr` package).
...
Ok, let's now get started with `tidyr`.
When I'm first learning a package, I often start by pulling up the documentation for it. You can do this by running `library(help="packagename")`, like this:
```{r}
library(help="tidyr")
```
...
In this case, you will see that the package includes not only functions, but also sample datasets, which we will use to demonstrate some functions. 
